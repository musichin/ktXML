...
import com.github.musichin.ktxml.elementOf
import org.assertj.core.api.Assertions.assertThat
import org.junit.Test

class PathTest {
    val element =
        elementOf("author-namespace", "root") {
            attribute("author-namespace", "author", "Very important author")
            element("author-namespace", "books") {
                attribute("author-namespace", "count", "10")
                for (index in 0..9) {
                    element("author-namespace", "book") {
                        text("title$index")
                        attribute("author-namespace", "rating", "${index % 5}")
                    }
                }
            }
    ...
    fun testNamespace() {
        ...
        assertThat(empty / XNamespace).isEqualTo("")
    ...
    fun testPaths() {
        ...
        assertThat(
            element / XPaths("author-namespace", "books") /
                XPaths(
                    "author-namespace",
                    "book",
                ) / XName,
        ).containsOnly("book")
        assertThat(element / XPath("author-namespace", "books") / XPaths("author-namespace", "book") / XName)
            .containsOnly("book")
        assertThat(element / XPaths("author-namespace", "books") / XPath("author-namespace", "book") / XName)
            .containsOnly("book")
    ...
    fun testPathsWithGetOperator() {
        ...
        assertThat(
            element / XPaths("author-namespace", "books")[0] /
                XPaths(
                    "author-namespace",
                    "book",
                )[2] / XText,
